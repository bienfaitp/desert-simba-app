...
-- ---- clean slate (safe on a fresh project; drops only our objects) ----
drop trigger if exists on_auth_user_created on auth.users;
drop table if exists
  media, staff_agreements, document_signatures, documents, announcements,
  coach_comments, match_stats, matches, attendance, events, fields,
  registrations, players, teams, seasons, user_roles, profiles cascade;
drop function if exists public.handle_new_user() cascade;
drop function if exists public.has_role(user_role) cascade;
drop function if exists public.coaches_team(uuid) cascade;
drop function if exists public.owns_player(uuid) cascade;
drop type if exists
  user_role, player_status, reg_status, event_type, attendance_status,
  announce_category, doc_type, media_type, bgcheck_status cascade;
