...
const BLANK = {
  ...
  ec2Name: "", ec2Phone: "",
  photoConsent: null,   // must be an explicit yes or no
  acknowledged: false,
  signature: "",
  payChoice: null,      // "pay_now" | "pay_later"
};
...
function RegistrationWizard({ parentName, onCancel, onDone }) {
  ...
  async function submit() {
    ...
    // Payment-ready: if the parent chose "pay now" we mark it paid via the
    // placeholder method; a real provider will set payment_reference later.
    ...
    const { error } = await supabase.from("registrations").insert({
      ...
      // One acknowledgment covers the whole agreement; these flags mirror the
      // sections of the document it covers.
    ...
    setBusy(false);
    if (error) {
      // Most likely cause on a fresh deploy: migration 0002 hasn't been run yet.
      const missingColumn = /column .* does not exist|schema cache/i.test(error.message);
      return setErr(missingColumn
        ? "The club database hasn't been updated for the new registration form yet. Ask your club admin to run migration 0002 in Supabase, then try again."
        : error.message);
    }
    onDone();
  }
...
function StepChild({ f, set }) {
  return (
    <>
      <div className="eyebrow" style={{ marginBottom: 4 }}>Step 1 of 4</div>
...
function StepMedical({ f, set }) {
  return (
    <>
      <div className="eyebrow" style={{ marginBottom: 4 }}>Step 2 of 4</div>
...
function StepContacts({ f, set }) {
  return (
    <>
      <div className="eyebrow" style={{ marginBottom: 4 }}>Step 3 of 4</div>
...
function StepAgreements({ f, set, parentName }) {
  return (
    <>
      <div className="eyebrow" style={{ marginBottom: 4 }}>Step 4 of 4</div>
      ...
      <div className="sect">
        ...
        {PHOTO_CHOICES.map((c) => (
          <label key={String(c.value)} className={"agree" + (f.photoConsent === c.value ? " on" : "")}
            style={{ display: "block", cursor: "pointer" }}>
            <div className="agree-head" style={{ cursor: "pointer" }}>
              <input type="radio" name="photoConsent" checked={f.photoConsent === c.value}
                onChange={() => set({ photoConsent: c.value })} style={{ borderRadius: "50%" }} />
  ...
  return (
    <>
      ...
      {free ? (
        <>
          <p className="muted" style={{ fontSize: 14, marginBottom: 16 }}>
            There's no fee to register right now. Tap below to finish.
          </p>
        ...
        <>
          <div className="fee-box">
            ...
            <b>{money(fee.cents, fee.currency)}</b>
          </div>
